...
After Tatoeba downloads + Pimsleur extraction + lexide-preprocess have all
finished, the local `data/audio/<lang>/` directories contain:
  ...
  - phonemes.jsonl     (regenerated by lexide-preprocess)
...
the sky_*.yaml files mount `.work/pron_audio.tar` (built by lexide-preprocess)
