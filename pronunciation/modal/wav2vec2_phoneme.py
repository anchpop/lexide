# Relocated from yap: /data/coding/yap/modal-envs/wav2vec2_phoneme.py
# Source file revision: d2c667218a5bb8ebf4fafad1f0ef5a83fcd5f5a7
# Source yap HEAD: d2c667218a5bb8ebf4fafad1f0ef5a83fcd5f5a7
# Inference behavior is preserved; canonical implementations are noted below.
...
# Production app name. The eval harness in the sibling yap repo
# (generate-data/src/bin/compare-audio-models.rs) overrides this via
# WAV2VEC2_APP_NAME so model comparisons deploy to a *separate* app
...
DECODER_VERSION = "nonblank_v1"


def _response_identity() -> dict:
    """Shared identity schema for HTTP single/batch and explicit error envelopes.

    All four fields are strings; empty phoneme lists carry the same identity.
    Single success: {identity..., phonemes, optional diagnostics/scores}.
    Batch: {identity..., results: [RPC success or {error}]}.
    Marker probe: {identity..., language_head_specs} or {identity..., load_error}.
    Explicit HTTP 422: {identity..., detail: str}; model-load HTTP 503:
    {detail: {identity..., load_error}}. RPC validation/load failures still raise.
    Framework validation errors retain their existing HTTP detail schema.
    RPC results and batch items keep their existing schemas without identity;
    nested scores, frames, and matrices are payloads, not response envelopes.
    """
    return {
...
class Wav2Vec2Phoneme:
    ...
    def _phone_frames(self, log_probs, p_nonblank):
        ...
        import torch
        # Canonical frame decision: pronunciation/inference/infer.py::decide_frames.
        # This hosted implementation retains its sigmoid > 0.5 gate and
        # checkpoint-masked phone slots; do not change numerics during relocation.
        # DECODER_VERSION identifies this nonblank-first wire behavior.
    ...
    def predict_batch(self, request: dict) -> dict:
        ...
        return {"results": self.transcribe_batch.local(requests), **_response_identity()}
    ...
    def predict(self, request: dict) -> dict:
        ...
        if self.load_error is not None:
            if request.get("marker_only"):
                return {"load_error": self.load_error, **_response_identity()}
            from fastapi import HTTPException
            raise HTTPException(
                status_code=503,
                detail={"load_error": self.load_error, **_response_identity()},
            )
        ...
        if isinstance(result, Exception):
            from fastapi.responses import JSONResponse
            return JSONResponse(status_code=422, content={
                "detail": str(result), **_response_identity(),
            })
        ...
        result.update(_response_identity())
        return result
