# Copied from yap/modal-envs/tests/test_pronunciation_batching.py
# Original last-change revision: d2c667218a5bb8ebf4fafad1f0ef5a83fcd5f5a7
...
def assert_identity(envelope):
    assert envelope["deploy_marker"] == service.DEPLOY_MARKER
    assert envelope["model_id"] == service.MODEL_ID
    assert envelope["model_revision"] == service.MODEL_REVISION
    assert envelope["decoder_version"] == "nonblank_v1"


@pytest.fixture
def http_client(worker):
    worker.language_head_specs = {"thai": {"lang": "tha", "target": "tone"}}
    worker.transcribe_batch = SimpleNamespace(local=lambda requests: batch_method(worker, requests))
    predict_endpoint = Service.predict._get_raw_f()
    app = FastAPI()

    @app.post("/single")
    def single(request: dict):
        return predict_endpoint(worker, request)

    @app.post("/batch")
    def batch(request: dict):
        return batch_endpoint(worker, request)
...
def test_prediction_envelopes_identify_model_and_decoder(worker, http_client, route, empty):
    if empty:
        forward = worker._forward_requests

        def silence(items):
            lp, stress, nb, aux, lengths = forward(items)
            nb.zero_()
            return lp, stress, nb, aux, lengths

        worker._forward_requests = silence
    request = clip(1600, return_frames=True, return_frame_matrix=True)
    payload = {"requests": [request]} if route == "/batch" else request
    response = http_client.post(route, json=payload)
    assert response.status_code == 200
    envelope = response.json()
    assert_identity(envelope)
...
def test_marker_only_identifies_decoder_without_inference(worker, http_client):
    def unexpected_inference(requests):
        pytest.fail("marker_only must not run inference")

    worker._transcribe_requests = unexpected_inference
    response = http_client.post("/single", json={"marker_only": True})
    assert response.status_code == 200
    assert_identity(response.json())
    assert response.json()["language_head_specs"] == worker.language_head_specs


@pytest.mark.parametrize("route,payload,status", [
    ("/single", {"marker_only": True}, 200),
    ("/single", clip(1600), 503),
    ("/batch", {"requests": [clip(1600)]}, 503),
])
def test_load_error_envelopes_identify_model_and_decoder(worker, http_client, route, payload, status):
    worker.load_error = "failed to load test model"
    response = http_client.post(route, json=payload)
    assert response.status_code == status
    envelope = response.json() if status == 200 else response.json()["detail"]
    assert envelope["load_error"] == worker.load_error
    assert_identity(envelope)


@pytest.mark.parametrize("requests,errors", [
    ([clip(1600), {}, clip(2000)], [False, True, False]),
    ([{}, {"audio": []}], [True, True]),
])
def test_batch_item_errors_retain_envelope_identity(http_client, requests, errors):
    response = http_client.post("/batch", json={"requests": requests})
    assert response.status_code == 200
    envelope = response.json()
    assert_identity(envelope)
    assert ["error" in item for item in envelope["results"]] == errors
...
@pytest.mark.parametrize("route,payload", [
    ("/single", {}),
    ("/single", {"audio": []}),
    ("/single", clip(1600, top_k="bad")),
    ("/batch", {}),
    ("/batch", {"requests": []}),
    ("/batch", {"requests": [{}] * 65}),
    ("/batch", {"requests": "bad"}),
])
def test_handler_validation_errors_identify_model_and_decoder(http_client, route, payload):
    response = http_client.post(route, json=payload)
...
@pytest.mark.parametrize("route", ["/single", "/batch"])
def test_forward_failure_envelopes_identify_model_and_decoder(worker, http_client, error_type, route):
    def failing_forward(items):
        raise error_type("simulated forward failure")

    worker._forward_requests = failing_forward
    request = clip(1600)
    payload = {"requests": [request]} if route == "/batch" else request
    response = http_client.post(route, json=payload)
    assert response.status_code == (200 if route == "/batch" else 422)
    envelope = response.json()
    assert_identity(envelope)
    if route == "/batch":
