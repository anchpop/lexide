...
`g2p_client.phonemize(text, lang, variety=...)`. g2p selects its implementation
...
It reads the dataset's variety (including historical Spanish/Portuguese
`espeak_voice` metadata), validates factor alignment, and applies the
...
`.cache/g2p_labels.sqlite3`, keyed by exact text, language, variety and g2p build
...
Use `--langs` to select a corpus subset. Unsupported language/variety requests
