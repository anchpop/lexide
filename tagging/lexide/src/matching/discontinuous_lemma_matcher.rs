...
/// ignoring POS tags on the anchors. For example, pattern `["ne", "que"]` will match
/// "ne peux dire que" because "ne" and "que" appear in order regardless of their POS tags.
///
/// Supports:
...
impl<K: Clone> DiscontinuousLemmaMatcher<K> {
    /// Creates a new discontinuous lemma matcher from lemma-only patterns.
    ///
    /// # Arguments
    ///
    ...
    pub fn find_all(&self, tokenization: &Tokenization) -> Vec<DiscontinuousMatch<K>> {
        ...
        'pattern: for (idx, (label, anchors)) in self.patterns.iter().enumerate() {
            ...
            for start in 0..lemma_seq.len() {
                if lemma_seq[start] != anchors[0].lemma {
                    continue;
                }
                ...
                for anchor_idx in 1..anchors.len() {
                    ...
                    let search_end = if let Some(max_gap) = self.max_gap {
                        (prev_pos + 1 + max_gap).min(lemma_seq.len())
                    } else {
                        lemma_seq.len()
                    };
...
mod tests {
    ...
    #[test]
    fn test_matches_regardless_of_pos() {
        let patterns = vec![("ne_que".to_string(), vec!["ne", "que"])];
        let matcher = DiscontinuousLemmaMatcher::new(&patterns, None);
