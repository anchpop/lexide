...
def _replace_clippable_linear(model):
    """Replace Gemma4ClippableLinear wrappers with their inner nn.Linear so PEFT can wrap them."""
    try:
    ...
    for name, module in model.named_modules():
        for child_name, child in module.named_children():
            if isinstance(child, Gemma4ClippableLinear):
                # Gemma4ClippableLinear wraps an nn.Linear in self.linear
                setattr(module, child_name, child.linear)
                count += 1
    if count:
        print(f"Replaced {count} Gemma4ClippableLinear wrappers with inner nn.Linear for PEFT compatibility")
    return model


def setup_lora(model, lora_args: LoraArguments):
    model = _replace_clippable_linear(model)
