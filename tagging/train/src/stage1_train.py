...
def build_char_labels(paragraph, lang_code=None):
    """Build character-level labels + dependency annotations for a paragraph."""
    no_space_langs = {"jpn", "zho"}
    use_space_joiner = lang_code not in no_space_langs
    ...
    for sent_idx, sent in enumerate(paragraph):
        ...
        # (skip for Japanese/Chinese which don't use inter-sentence spaces)
        if sent_idx > 0 and use_space_joiner:
...
class Stage1Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        lang_code = paragraph[0].get("lang") if paragraph else None
        text, bio_pos_ids, sent_start_ids, dep_annotations = build_char_labels(paragraph, lang_code=lang_code)
